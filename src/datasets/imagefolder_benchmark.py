...
def collect_imagefolder_records(root: Path, split: str, class_to_idx: Dict[str, int] | None = None):
...
def build_imagefolder_class_incremental_benchmark(benchmark_name: str, config: Dict[str, object]) -> ContinualBenchmark:
    benchmark_cfg = config["benchmark"]
    root = Path(str(benchmark_cfg["data_root"]))
    train_records, class_to_idx = collect_imagefolder_records(root, "train")
    test_records, _ = collect_imagefolder_records(root, "test", class_to_idx=class_to_idx)
    ...
    for task_id, class_ids in enumerate(class_splits):
        tasks.append(
            TaskDefinition(
                ...
                metadata={"benchmark": benchmark_name, "class_to_idx": class_to_idx},
            )
        )
